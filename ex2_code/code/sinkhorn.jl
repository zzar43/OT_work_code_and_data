...
function sinkhorn_1d_old(a, b, M, reg; iterMax=100, errThr=1e-2, verbose=true)
    a = a ./ norm(a,1)
    b = b ./ norm(b,1)
...
function update_v!(v, b, K, u)
    v = b ./ (K*u)
end
function update_u!(u, K_tilde, v)
    u = 1 ./ (K_tilde*v)
end

# Unbalanced Sinkhorn OT
function unbalanced_sinkhorn_1d_old(a, b, M, reg, reg_m; iterMax=100, verbose=true)
    Na = length(a)
    Nb = length(b)
    size_M = size(M)
    if Na != size_M[1] || Nb != size_M[2]
